...
$routes->group('ppdb', ['namespace' => 'Ppdb\Controllers'], function ($routes) {
    ...
    $routes->post('pendaftar/save-pembayaran', 'Pendaftar::save_pembayaran');
    ...
    $routes->post('berkas/syarat-save', 'Berkas::syarat_save');
    ...
    $routes->post('berkas/update-status', 'Berkas::update_status');

    // Penjadwalan Tes
    $routes->get('jadwal', 'JadwalTes::index');
    $routes->post('jadwal/save', 'JadwalTes::save');
    $routes->get('jadwal/delete/(:num)', 'JadwalTes::delete/$1');
    $routes->get('jadwal/detail/(:num)', 'JadwalTes::detail/$1');
    $routes->post('jadwal/add-peserta', 'JadwalTes::add_peserta');
    $routes->get('jadwal/remove-peserta/(:num)', 'JadwalTes::remove_peserta/$1');
    $routes->post('jadwal/update-kehadiran', 'JadwalTes::update_kehadiran');

    // Public Side (Front-end)
    $routes->get('daftar', 'PublicPendaftaran::index');
    $routes->post('daftar/submit', 'PublicPendaftaran::submit');
